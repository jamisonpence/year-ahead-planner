...
import { RefreshCw, X, ChevronLeft, ChevronRight, Dumbbell, CheckCircle2, ArrowLeftRight } from "lucide-react";
...
export default function GeneralFitnessWizard({ open, onClose, onSaved, defaultGoal }: { open: boolean; onClose: () => void; onSaved?: () => void; defaultGoal?: string }) {
  ...
  const [swapTarget, setSwapTarget] = useState<{weekLabel: string; di: number; role: 'primary'|'accessory'; ai: number; focus: string; marker: string} | null>(null);
  ...
  function swapExercise(replacement: any) {
    if (!swapTarget || !plan) return;
    const { weekLabel, di, role, ai, marker } = swapTarget;
    const newPlan = JSON.parse(JSON.stringify(plan));
    const day = newPlan.weeks[weekLabel][di];
    const newEx = { ...replacement, marker, sets: prescription(replacement, role, marker, Math.max(45, parseInt(bench) || 185)) };
    if (role === 'primary') {
      day.primary_lift = newEx;
    } else {
      day.accessories[ai] = newEx;
    }
    setPlan(newPlan);
    setSwapTarget(null);
    setSwapSearch('');
  ...
  function getSwapOptions() {
    ...
    const { focus: sessionFocus, role } = swapTarget;
    const avail = availableRepoEquipment(equipment);
    return exerciseRepository.filter(ex => {
      if (!matchesEquipment(ex, equipment)) return false;
      if (ex.focus_area !== sessionFocus) return false;
      const q = swapSearch.toLowerCase();
      if (q && !ex.name.toLowerCase().includes(q) && !ex.primary_muscle_group.toLowerCase().includes(q) && !ex.movement_pattern.toLowerCase().includes(q)) return false;
      return true;
    });
  }
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
                    ...
                    <div key={di} className="border-t px-3 py-3">
                      ...
                      <div className="mb-2">
                        <div className="flex items-center justify-between gap-2 mb-0.5">
                          ...
                          <button type="button" onClick={() => { setSwapTarget({ weekLabel, di, role: 'primary', ai: 0, focus: day.session_type, marker: day.marker }); setSwapSearch(''); }}
                            className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground border border-border rounded px-1.5 py-0.5 hover:bg-secondary transition-colors shrink-0">
                      ...
                      {day.accessories.map((acc: any, ai: number) => (
                        <div key={ai} className="mt-2 pt-2 border-t border-border/50">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button type="button" onClick={() => { setSwapTarget({ weekLabel, di, role: 'accessory', ai, focus: day.session_type, marker: acc.marker }); setSwapSearch(''); }}
                              className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground border border-border rounded px-1.5 py-0.5 hover:bg-secondary transition-colors shrink-0">
                              <ArrowLeftRight size={10} /> Swap
                            </button>
                          </div>
      ...
      {/* Swap Picker */}
      {swapTarget && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 p-4" onClick={() => setSwapTarget(null)}>
          <div className="bg-background border rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[70vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b shrink-0">
              <p className="text-sm font-semibold">Swap Exercise</p>
              <button onClick={() => setSwapTarget(null)} className="p-1 rounded hover:bg-secondary"><X size={14} /></button>
            </div>
            <div className="px-4 py-2 shrink-0">
              <Input autoFocus placeholder="Search by name, muscle, or pattern…" value={swapSearch} onChange={e => setSwapSearch(e.target.value)} className="h-8 text-sm" />
            </div>
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-1">
              {getSwapOptions().length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-6">No exercises match your equipment and search.</p>
              )}
              {getSwapOptions().map(ex => (
                <button key={ex.name} type="button" onClick={() => swapExercise(ex)}
                  className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-secondary transition-colors border border-transparent hover:border-border">
                  <p className="text-sm font-medium">{ex.name}</p>
                  <p className="text-[11px] text-muted-foreground">{ex.movement_pattern} · {ex.primary_muscle_group}</p>
                </button>
              ))}
            </div>
          </div>
